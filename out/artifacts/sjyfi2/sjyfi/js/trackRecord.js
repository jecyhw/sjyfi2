            ...
            clipMap = $("#map").imgAreaSelect({
                ...
                onSelectChange: function (it, selection) {
                    ...
                    var url = "http://api.map.baidu.com/geoconv/v1/?from=1&to=5&ak=kPEqHXhcmEDm9DhNYKbKfeer&callback=?&coords=" +
                            _topLeft.lng + "," + _topLeft.lat + ";" +
                            _bottomRight.lng + "," + _bottomRight.lat;
                    $.ajax({
                        url: url,
                        type: "post",
                        dataType: "json",
                        ...
                        success: function (data) {
                            if (data.status == 0) {
                                var _topLeftNew = data.result[0],
                                _bottomRightNew = data.result[1];
                                $("#left" + index).val(_topLeft.lng * 2 - _topLeftNew.x);
                                $("#top" + index).val(_bottomRight.lat * 2 - _topLeftNew.y);
                                $("#right" + index).val(_bottomRight.lng * 2 - _bottomRightNew.x);
                                $("#bottom" + index).val(_topLeft.lat * 2 - _bottomRightNew.y);
                            }
                        }
                    });
                }
